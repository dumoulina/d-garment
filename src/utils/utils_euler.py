...
* http://torchworld.wolfram.com/EulerAngles.html
...
in the vector :torch:`alpha`, :torch:`beta` and :torch:`gamma`.  The vector is [
:torch:`alpha`, :torch:`beta`. :torch:`gamma` ] and, in this description, the
order of the parameters specifies the order in which the rotations occur (so
the rotation corresponding to :torch:`alpha` is applied first).


In order to specify the meaning of an *Euler angle vector* we need to specify
the axes around which each of the rotations corresponding to :torch:`alpha`,
:torch:`beta` and :torch:`gamma` will occur.

There are therefore three axes for the rotations :torch:`alpha`, :torch:`beta`
and :torch:`gamma`; let's call them :torch:`i` :torch:`j`, :torch:`k`.

Let us express the rotation :torch:`alpha` around axis `i` as a 3 by 3 rotation
matrix `A`.  Similarly :torch:`beta` around `j` becomes 3 x 3 matrix `B` and
:torch:`gamma` around `k` becomes matrix `G`.  Then the whole rotation expressed
by the Euler angle vector [ :torch:`alpha`, :torch:`beta`. :torch:`gamma` ], `R`
is given by::

   R = np.dot(G, np.dot(B, A))

See http://torchworld.wolfram.com/EulerAngles.html

The order :torch:`G B A` expresses the fact that the rotations are
performed in the order of the vector (:torch:`alpha` around axis `i` =
`A` first).
...
convention : http://torchworld.wolfram.com/EulerAngles.html.
...
def euler2mat(p, axes='sxyz'):
    ...
    M = torch.eye(3, device=ai.device)[
        None, :, :].repeat(ai.shape[0], 1, 1)
...
def mat2euler(M, axes='sxyz'):
    ...
    Parameters
    ----------
    M : array-like shape (3, 3) Rotation matrix
    axes : str, optional
        Axis specification; one of 24 axis sequences as string or encoded
        tuple - e.g. ``sxyz`` (the default).
    ...
    if repetition:
        ...
        az = sy.clone()
        if sy > _EPS4:
            ax[sy > _EPS4] = torch.atan2(
                M[:, i, j][sy > _EPS4],  M[:, i, k][sy > _EPS4])
            ay[sy > _EPS4] = torch.atan2(
                sy[sy > _EPS4],       M[..., i, i][sy > _EPS4])
            az[sy > _EPS4] = torch.atan2(
                M[..., j, i][sy > _EPS4], -M[..., k, i][sy > _EPS4])
        else:
            ax = torch.atan2(-M[..., j, k][sy <= _EPS4],
                             M[..., j, j][sy <= _EPS4])
            ay = torch.atan2(sy[sy <= _EPS4],       M[..., i, i][sy <= _EPS4])
            az = 0.0
    else:
        ...
        ax[cy > _EPS4] = torch.atan2(
            M[..., k, j][cy > _EPS4],  M[..., k, k][cy > _EPS4])
        ay[cy > _EPS4] = torch.atan2(-M[..., k, i]
                                     [cy > _EPS4],  cy[cy > _EPS4])
        az[cy > _EPS4] = torch.atan2(
            M[..., j, i][cy > _EPS4],  M[..., i, i][cy > _EPS4])

        ax[cy <= _EPS4] = torch.atan2(-M[..., j, k]
                                      [cy <= _EPS4],  M[..., j, j][cy <= _EPS4])
        ay[cy <= _EPS4] = torch.atan2(-M[..., k, i]
                                      [cy <= _EPS4],  cy[cy <= _EPS4])
        az[cy <= _EPS4] = 0.0
    ...
    if frame:
        ax, az = az, ax
    return torch.cat((ax[..., None], ay[..., None], az[..., None]), dim=-1)
